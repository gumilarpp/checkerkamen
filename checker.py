...
NFTOKEN_API_URL = "https://ios.prod.ftl.netflix.com/iosui/user/16.8"
NFTOKEN_PARAMS = {
    "appVersion": "16.8.0",
    "device_type": "NFAPPL-02-",
    "esn": "NFAPPL-02-IPHONE14=1-PXA-A2111-D4F5B3A6E7C8D9E0F1A2B3C4D5E6F7A8B9C0D1E2F3A4B5C6",
    "idiom": "phone",
    "iosVersion": "17.5.1",
    "pathFormat": "graph",
    "responseFormat": "json",
    "path": '["account","token","default"]',
    "config": json.dumps({"device_type": "NFAPPL-02-", "idiom": "phone", "iosVersion": "17.5.1", "appVersion": "16.8.0"}),
}
NFTOKEN_HEADERS = {
    "User-Agent": "Argo/16.8.0 (iPhone; iOS 17.5.1; Scale/3.00)",
    "Accept": "application/json",
    "Accept-Language": "en-US,en;q=0.9",
    "x-netflix.client.type": "argo",
    "x-netflix.request.routing": json.dumps({"path": "/nq/mobile/nqios/~16.8.0/user", "control_tag": "iosui_argo"}),
    "x-netflix.context": "{}",
    "x-netflix.profiles": "{}",
    "x-netflix.app-version": "16.8.0",
    "x-netflix.idiom": "phone",
    "x-netflix.os.version": "17.5.1",
    "x-netflix.device.model": "iPhone14,2",
}
...
def generate_nftoken(cookies: dict) -> dict:
    ...
    try:
        # Build cookie header — include all session cookies available
        cookie_parts = [f"NetflixId={netflix_id}"]
        secure_id = cookies.get("SecureNetflixId") or cookies.get("secureNetflixId") or ""
        nfvdid    = cookies.get("nfvdid") or ""
        if secure_id:
            cookie_parts.append(f"SecureNetflixId={secure_id}")
        if nfvdid:
            cookie_parts.append(f"nfvdid={nfvdid}")
        cookie_str = "; ".join(cookie_parts)
        headers = {**NFTOKEN_HEADERS, "Cookie": cookie_str}
        if _CURL_AVAILABLE:
            # Use curl_cffi to match Chrome TLS fingerprint — avoids Netflix 500
            import curl_cffi.requests as _cffi
            sess = _cffi.Session(impersonate="chrome131")
            resp = sess.get(NFTOKEN_API_URL, params=NFTOKEN_PARAMS, headers=headers, timeout=10)
        else:
            resp = requests.get(NFTOKEN_API_URL, params=NFTOKEN_PARAMS, headers=headers, timeout=10)
...
def _make_session(bulk_mode: bool):
    ...
    """
    if _CURL_AVAILABLE:
        s = _curl_requests.Session(impersonate="chrome131")
            ...
            for attempt in range(max_attempts):
                try:
                    ...
                    if attempt == max_attempts - 1:
                        logger.warning("_fetch_account failed (url=%s attempt=%d): %s", url, attempt, exc)
                        break
