        ...
        try:
            ...
            if nf_id:
                nft = await loop.run_in_executor(
                    _EXECUTOR, generate_nftoken, {
                        "NetflixId":       nf_id,
                        "SecureNetflixId": result.get("secure_netflix_id", ""),
                        "nfvdid":          result.get("nfvdid", ""),
                    }
                )
                result["nftoken"] = nft
    ...
    try:
        with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
            ...
            for i, (result, src, _) in enumerate(premium, 1):
                ...
                fname = f"Premium Hits/{i:02d}_{safe}_{plan}.txt"
            ...
            for i, (result, src, _) in enumerate(normal, 1):
                ...
                fname = f"Normal Hits/{i:02d}_{safe}_{plan}.txt"
...
async def cancel_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Cancel the user's currently running check session."""
    uid = update.effective_user.id if update.effective_user else None
    if not uid:
        return
    session_id = _USER_SESSION.get(uid)
    if session_id:
        _CANCEL_SESSIONS.add(session_id)
        await update.message.reply_text(
            "🛑 <b>Cancellation requested.</b>\n"
            "<i>Your active check will stop after the current batch.</i>",
            parse_mode=ParseMode.HTML,
        )
    else:
        await update.message.reply_text(
            "ℹ️ <b>No active check to cancel.</b>",
            parse_mode=ParseMode.HTML,
        )
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text(
        ...
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        "⚙️ <b>Output Modes</b>\n"
        "  /basic    — Compact card view <b>(default)</b>\n"
        "  /fullinfo — All fields, full detail\n"
...
async def setmode_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        await query.answer(f"Mode set to {label} ✅")
...
async def process_cookie_sets(
    ...
    for batch_start in range(0, total, BULK_CONCURRENCY):
        ...
        for batch_idx, ((src, cs, raw), result) in enumerate(zip(batch, batch_results)):
            ...
            if status == "hit":
                ...
                hits_store.save_hit(result, raw)
            elif status == "free":
                frees += 1
                hits_list.append((result, src, raw))
                hits_store.save_hit(result, raw)
    ...
    if is_bulk:
        ...
        if hits_list:
            try:
                # Show a brief "building ZIP" notice while tokens + ZIP are generated
...
def _build_application() -> "Application":
    """Build and register all handlers on the PTB Application."""
    builder = (
        Application.builder()
        .token(BOT_TOKEN)
    ...
    # Proxy support: set HTTPS_PROXY env var to use a proxy
    # Example: HTTPS_PROXY=socks5://127.0.0.1:1080
    #          HTTPS_PROXY=http://user:pass@host:port
    # run.py already manages proxy env vars (clears system proxy if unused).
    # Pass explicit proxy to PTB so it doesn't fall back to trust_env discovery.
...
async def _common_startup(application: "Application") -> None:
    """Tasks that run regardless of polling vs webhook mode."""
    ...
    await application.bot.set_my_commands([
        ...
        BotCommand("cancel",   "Cancel your active check"),
    ])
    asyncio.create_task(_session_watchdog())


async def _run_webhook(application: "Application", webhook_url: str) -> None:
    """Run in webhook mode — for production (autoscale) deployments."""
    async with application:
        ...
        await _common_startup(application)
        print(f"✅ Webhook set → {webhook_url}")
        print("✅ Netflix Cookie Checker Bot is running (webhook mode)…")
